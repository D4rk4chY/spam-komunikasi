...
mulai (){
    echo "Gunakan SPAM KOMUKASI TOOLS Lagi?"
    echo "y/n?"
    echo
    read lagi
    if [ $lagi = "y" ]; then
            ...
            echo "Terimakasih sudah SPAM KOMUKASI TOOLS"
    fi
}
#spam
spam(){
    ...
    echo "[1] Spam SMS"
    echo "[2] Spam Telp"
    echo "[3] Close SPAM KOMUKASI TOOLS"
    echo
    echo "1/2/3?"
    read pilih
    if [ $pilih = "1" ]; then
...
echo
echo "Mulai Mulai SPAM KOMUNIKASI TOOLS?"
echo "y/n?"
read mulai
if [ $mulai = "y" ]; then
      spam
